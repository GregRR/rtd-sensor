...
- retain calibration provenance and explicitly distinguish unmodeled reference-temperature uncertainty from resistance weighting;
...
Weighted least squares may use either a positive relative `weight` on every observation or a positive `standard_uncertainty_ohms` on every observation. Resistance standard uncertainties are converted to normalized inverse-variance weights. Calibration/reference-temperature standard uncertainty is represented separately as `standard_uncertainty_temperature_c`; because temperature is the independent variable, current least-squares fitters reject that field by default rather than silently converting it into resistance weighting. Callers may explicitly choose `temperature_uncertainty_handling="retain_not_used"` to preserve the supplied temperature uncertainty in fit evidence while acknowledging that it is not included in the fit or fitted-parameter covariance. Modeling uncertainty in both coordinates requires an errors-in-variables treatment and remains outside the current API.

Absolute resistance standard uncertainties define parameter covariance directly under the fit assumptions, including for saturated fits. Unweighted and relative-weighted fits instead estimate the unknown common residual scale from residual scatter, so covariance is unavailable when residual degrees of freedom are zero. Polynomial fits and CVD fits that estimate any shape coefficient may narrow the fitted model's validity range inside the observed calibration span but may not extend beyond it. Characterized IEC `R0` fits, and CVD `R0`-only fits with independently fixed shape coefficients, retain the separate applicability-range semantics described above and may use a caller-declared range independent of the observation span. Rank-deficient, severely ill-conditioned, non-positive, or non-monotonic fitted curves raise `RTDFitError` instead of returning a deployable model.

Calibration record context can be retained separately with `fitting.CalibrationProvenance`, including a certificate identifier, calibration date/laboratory, reference standard, source document, and notes. This provenance is fit evidence only: it does not change the numerical model, is not copied into `coefficient_source`, and is not automatically embedded into portable model metadata.
...
No confidence level is inferred from a coverage factor. A statement such as `k = 2` only has a probability interpretation when that interpretation is justified by the complete uncertainty analysis. Correlated components are not supported by this general combination helper. Covariance-aware fitted-parameter propagation is available separately through `propagate_fit_covariance_to_resistance()` and `propagate_fit_covariance_to_temperature()`.
